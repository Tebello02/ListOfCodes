...
class aLargeInteger
{
	...
	friend std::istream& operator >> (std::istream & input, aLargeInteger & right);
	/*	Inputs a large integer on the right, a digit at the time, via standard input stream e.g. cin >> X */

	friend std::ostream& operator << (std::ostream & output, aLargeInteger right);
	/*	Outputs a large integer on the right on the standard output stream e.g. cout << X */

	friend std::fstream& operator >> (std::fstream & input, aLargeInteger & right);
	/*	Reads a large integer on the right from a text file, input. E.g. input >> right */

	friend std::fstream& operator << (std::fstream & output, aLargeInteger right);
	/*	Writes a large integer on the right to a text file, output. E.g. output << right */
	...
	*/
	...
	public:
		...
		*/
		...
		// E x t e r n a l   S t o r a g e   M e m b e r   F u n c t i o n s
				
		void write(std::fstream & output, int binary = 1);
		/*	It assumes that the output stream is open.
		
			It writes the large integer to the output file as follows:
			(1) First it writes the length of the large integer then
			(2) the sign (of the integer), and finally
			(3) all the digits of the integer
		
			The writing is either "binary" (which is the default) or "text" mode, according to the variable "binary".
			
		*/
		
		void read(std::fstream & input, int binary = 1);
		/*	It assumes that the input stream is open.
		
			It reads the large integer from the input file as follows:
			(1) First it reads the length of the large integer, then
			(2) the sign (of the integer), and finally
			(3) all the digits of the integer
		
			The reading is either "binary" (which is the default) or "text" mode, according to the variable "binary".
		*/
		
		void save();
		/*	It allows the user to save the large integer to a file of his/her choice.
			The file will be opened in "binary" or "text" mode and will overwrite any existing data in the fie.
		*/
		
		void read();
		/*	It allows the user to read the large integer from a file of his/her choice.
...
aLargeInteger :: aLargeInteger(long int _length)
{
	...
	{
		std::cout << "Cannot Allocate Space for the Digit Array ... ABORTING!\n";
		exit(0);
	}
}
...
void aLargeInteger :: resize(long int _length)
{
	...
	aLargeInteger newNum(_length);	//	Create a new number with the new length
...
aLargeInteger aLargeInteger :: operator = (aLargeInteger right)
{
	try
	{
		...
		this->digit = new int [right.length];	//	"new" might cause an exception

		for(long int i = 0; i < right.length; i++)
			this->digit[i] = right.digit[i];

	}
	catch(...)	//	Abort if there is an exception when allocating memory
	{
		std::cout << "Cannot Allocate Space for the Digit Array ... ABORTING!\n";
		exit(0);
	}
...
std::istream & operator >> (std::istream & input, aLargeInteger & right)
{
	//	Character constants
...
std::ostream& operator << (std::ostream & output, aLargeInteger right)
{
	if (right.sign < 0)
		output << "-";
...
std::fstream& operator >> (std::fstream & input, aLargeInteger & right)
{
	if (input.good())
	{
		try
		{
			//	Read the size of the integer first and then the sign
			input >> right.length >> right.sign;

			// allocate memory for incoming digits
			right.digit = new int[right.length];
		...
		{
			std::cout << "File is Bad or Cannot Allocate Memory for Incoming Digits ... ABORTING!\n";
			exit(0);
		}
	}
	return input;
...
std::fstream& operator << (std::fstream & output, aLargeInteger right)
{
	if (output.good())
	{
		try
		{
			//	Write the size of the integer first and then the sign
			output << right.length << "\t" << right.sign << "\n";	
			//	Separate this header from the digits by a newline
			...
			for(long int i = 0; (i < right.length) && output.good(); i++)
			{
				...
				if (i < (right.length-1))
					output << char(32);	//	Separate the digits with a space
				else
					output << "\n";	//	Separate large integers with a newline
			}
		}
		catch(...)
		{
...
void aLargeInteger :: read (std::fstream & input, int binary)
{
	...
	if (input.good())
	{
		if (binary)
		{
			try
			{
				...
				//	then read the sign
				input.read((char *) & sign, sizeof(sign));

				 // allocate memory for incoming digits
				digit = new int[length];

				//	and, finally read the whole integer a digit at a time
				for(long int i = 0; (i < length) && input.good(); i++)
					input.read((char *) & digit[i], sizeof(digit[i]));
			}
			catch(...)
			{
				std::cout << "File is Bad or Cannot Allocate Memory for Incoming Digits ... ABORTING!\n";
		...
		else	//	file is in text mode, use overloaded operator >>
			input >> (*this);
	}
	this->trim();
}
...
void aLargeInteger :: write (std::fstream & output, int binary)
{
	//	Assume the stream is opened
	if (output.good())
	{
		if (binary)
		{
			try
			{
				...
				//	then write the sign
				output.write((char *) & sign, sizeof(sign));

				//	and, finally write the whole integer a digit at a time
				for(long int i = 0; (i < length) && output.good(); i++)
				output.write((char *) & digit[i], sizeof(digit[i]));
			}
			catch(...)
			{
				std::cout << "File is Bad ... ABORTING!\n";
		...
		else	//	text file, use overloaded operator, <<, on this
			output << (*this);
	}
}


void aLargeInteger :: save()
{
	std::fstream file;
	...
	do
	{
		std::cout << "Enter file name to Save the Large Integer to  ";
		std::cin >> file_name;
	}while (file_name == "");
	...
	if (file.good())
	{
		...
		std::cout << "Large Integer SAVED to File ==> " << file_name << " <==\n";
	}
...
void aLargeInteger :: read()
{
	...
	do
	{
		std::cout << "Enter file name to Read the large Integer from  ";
		std::cin >> file_name;
	}while (file_name == "");
	...
	if (file.good())
	{
		...
		std::cout << "Large Integer READ from File ==> " << file_name << " <==\n";
	}
...
int main()
{
	aLargeInteger X;// = 20;
	//std::cout << "X = "  << X << "\n";
	std::cout << "Enter any large number to test with = > " ;
	std::cin >> X;
	X = X ^ 999;

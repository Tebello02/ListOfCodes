...
//	set_all_Coord(...) will set all the coordinates of the point the corresponding values.
{
	if (_dimension > dimension)
		dimension = _dimension;	//	Clip the given value to the actual point's dimension
...
{
	//	C++ code here
	long double product, sum;

	if (this->dimension == right.dimension)
	{
		for (int i = 0; i<dimension; i++)
		{
			product = this->point[i] * right.point[i]; sum += product;
		}
	}
	else
	{
...
//	k * (x0, x1, x2, ..., xN-1) = (k*x0, k*x1, k*x2, ..., k*xN-1)
{
	...
	{		
		double value = left * right.get_Coord(i);	
		P.set_Coord(i, value);	//P.point[i] = left * right.point[i];
	}
...
std::istream & operator >> (std::istream & scan, aPoint &right)
...
std::ostream & operator << (std::ostream & print, aPoint right)
...
//	(x0, x1, x3), if P is a point in 3D space, etc
{
	...
	{
		print << right.point[right.dimension - 1];
	}	
	
	if (right.dimension >= 2)
	{
		std::cout << "(";
...
int main()
{
	aPoint	P1, 		// Declares P1 as a point 0 on the real line
					P2(2),	// Declares P2 as a 2D point with values: (0, 0)
					P3(3),	// Declares P3 as a 3D point with values: (0, 0, 0)
					P4(4);	// Declares P4 as a 4D point with values: (0, 0, 0, 0)
	...
	// Changing the i-th coordinate of a point
	std::cout << "Changing the i-th coordinate of a point: " << "\n";
	
	std::cout << "Initial values of Point4: " << (P4) << "\n";
	P4.set_Coord(0, 5); P4.set_Coord(1, 10); P4.set_Coord(2, 15); P4.set_Coord(3, 2);	
	std::cout << "Point4 after changing the 1st and 2nd coordinates: " << (P4) << "\n";
		
	//	Compute and display the scalar multiplaction of point P1 by the scalar 2 -- P1 * 2
	std::cout << P1 << " * " << 2 << " = " << (P1 * 2) << "\n";
	std::cout << 4 << " * " << P4 << " = " << (4.0 * P4) << "\n";
	P1  = P2;
	if (P1 == P2)
		std::cout << P1 << " == " << P2 << "\n";
	...
	std::cout << "Getting the value of the i-th coordinate: " << "\n";
	std::cout << "The 1st coordinate for Point 3 is: " << P3.get_Coord(1) << "\n"; /// std::cout << "\n";
	return 1;
}

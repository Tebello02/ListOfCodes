...
//	set_allCoord(...) will set the coordinates of the point the corresponding values.
{
...
//	Component values of P are read from the keysboard.
{
	...
	if (right.get_dimension() < 2)
	{
		std::cout << "Enter a point on the real line ";
		input >> value;
		right.set_Coord(0, value);	//	right.input[0] = value
	}
	else
	{
...
{
	if (right.get_dimension() < 2)
		output << right.get_Coord(0);	//	A point on the real line -- just display it
	else	//	More than one point? A 2D, 3D , ..., point, display it as: (x0, x1, x2, ..., xN-1))
	{
		output << "(";
		...
		for (long int i = 0; i < right.get_dimension(); i++)
		{
			...
			if (i < (right.get_dimension()-1))
				output << ", ";	//	separate coordinates with commas
		}
		output <<  ")";
	}

...
/*
	NAME: Class{a2D_Point__NO_FRIENDS}.cpp
	AUTHOR: mofana mphaka March 2021
	
	FUNCTION:
	
	The class shows how to use the
	
	(1) Set functions (e.g. set_xCoord(...)), and
	(2) Get functions (e.g. get_xCoord() ),
	
	when access to "private" data elements is required by non-member, non-friend functions/objects.
	
	Constructing a class for a point in a 2-Dimensional space -- a 2-D Point, without friend functions.
*/
...
class a2D_Point
{
	...
	public:
		...
		a2D_Point operator - (a2D_Point right);		//	subtract the point on the right from *this point -- *this - right
		...
		//	NB: There are no friend functions and/or friend classes declared. However, the following will be overloaded to work on objects of this class
		/*
				
			(1)	double distance(a2D_Point left, a2D_Point right);		//	Euclidean distance between the two points: left & right
			(2)	a2D_Point operator * (double left, a2D_Point right);	//	Scalar multiplication on the left: left * right
...
//	N o n - M e m b e r   a n d    n o n - f r i e n d    F u n c t i o n s
...
{
	double	xDiff = left.get_xCoord() - right.get_xCoord(),		//	get_xCoord() avails the "private" data, xCoord
			yDiff = left.get_yCoord() - right.get_yCoord();		//	get_yCoord() avails the "private" data, yCoord
...
//	Scalar Multiplication (on the left), k (x, y) is given by:
//
//	k * (x, y) = (k * x, k * y)
{
	a2D_Point P;
	
	P.set_xCoord(left * right.get_xCoord());	//	P.xCoord = left * right.xCoord;
	P.set_yCoord(left * right.get_yCoord());	//	P.yCoord = left * right.yCoord;
...
{
	double coord;	//	To hold coordinate data temporarily
	
	std::cout << "Enter new X coordinate ";
	input >> coord;
	right.set_xCoord(coord);	//	right.xCoord = coord
	
	std::cout << "Enter new Y coordinate ";
	input >> coord;
	right.set_yCoord(coord);	//	right.yCoord = coord
...
{
	output << "(" << right.get_xCoord() << ", " << right.get_yCoord() << ")";
